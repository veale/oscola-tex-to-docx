...
local function restructure_footnotes(dom)
  ...
  -- Step 1: Find the footnotes container
  local fn_divs = dom:query_selector(".footnotes")
  if #fn_divs == 0 then
    log:warning("No .footnotes div found — document may have no footnotes")
    return dom
  end
  local fn_div = fn_divs[1]

  -- Step 2: Extract footnote content from <aside class="footnotetext"> elements
  local footnotes = {}
  ...
  if #asides == 0 then
    ...
    log:info("No <aside> elements found, falling back to <p> elements")
  end
  ...
  for _, aside in ipairs(asides) do
    ...
    -- Remove HTML comments (they show up as text nodes starting with "<!--")
    -- The serialize approach handles this naturally
  ...
  if #footnotes == 0 then
    log:warning("Found .footnotes div but no extractable footnotes")
    return dom
  end

  log:info("Extracted " .. #footnotes .. " footnotes")

  -- Step 3: Remove the original footnotes div
  fn_div:remove_node()

  -- Step 4: Replace footnote references in the body text
  ...
  for _, mark in ipairs(marks) do
    local fn_num = mark:get_text():match("(%d+)")
    if fn_num then
      -- Create the new Pandoc-style structure using the DOM API
      ...
      local text_node = new_a:create_text_node(fn_num)
  ...
  -- Step 5: Build the Pandoc-style footnotes section
  -- Parse it from an HTML string using html_parse
